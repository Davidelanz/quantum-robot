...
def test_explicit_logging_configuration_writes_only_the_requested_file(
    ...
    try:
        ...
        assert (
            sum(getattr(handler, "_qrobot_managed", False) for handler in package_logger.handlers)
            == 1
        )
    finally:
...
def test_explicit_logging_configuration_can_write_to_console(capsys) -> None:
    ...
    package_logger = configure_logging(LoggingConfig(level=logging.WARNING, console=True))
