...
from .base_robot import MotionMode, Robot
from .config import PREY_CONFIG, PreyConfig
...
class BluePrey(Robot):
    ...
    :param config: Appearance, flight, and wandering parameters.
    :param rng: Independent random generator used by random motion.

    """

    motion_mode: MotionMode = "deterministic"
    config: PreyConfig = PREY_CONFIG
    ...
    def step(
        ...
        if self.distance_to(closest_hunter) < self.config.flee_distance:
    ...
    def _escape_command(self, hunter: Robot) -> tuple[float, float]:
        ...
        return self.config.flee_speed, self.turn_towards(escape_x, escape_y)
    ...
    def _wander_command(self, elapsed: float) -> tuple[float, float]:
        ...
        if self.motion_mode == "random" and elapsed >= self._next_choice:
            self._wander_turn = self.rng.uniform(*self.config.random_turn_range)
            self._next_choice = elapsed + self.rng.uniform(*self.config.random_choice_interval)
        turn = self._wander_turn if self.motion_mode == "random" else self.config.deterministic_turn
        return self.config.wander_speed, turn
