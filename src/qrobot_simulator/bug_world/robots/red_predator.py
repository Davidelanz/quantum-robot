...
from .config import PREDATOR_CONFIG, ROBOT_CONFIG, PredatorConfig
...
class RedPredator(Robot):
    ...
    :param config: Appearance, pursuit, and steering-noise parameters.
    :param rng: Independent random generator used by noisy pursuit.
    """

    motion_mode: MotionMode = "deterministic"
    bite_period: float = PREDATOR_CONFIG.bite_period
    config: PredatorConfig = PREDATOR_CONFIG
    ...
    def step(
        ...
        return self.move(self.config.pursuit_speed, turn, dt, bounds)
    ...
    def _pursuit_turn(self, target: Robot, elapsed: float) -> float:
        ...
        if self.motion_mode == "random" and elapsed >= self._next_choice:
            self._noise = self.rng.uniform(*self.config.random_noise_range)
